...
class TestFaceEncodingDataclass:
    ...
    def test_face_encoding_creation_with_required_fields(self):
    ...
    def test_face_encoding_creation_with_all_fields(self):
...
class TestFaceMatchDataclass:
    ...
    def test_face_match_creation_with_required_fields(self):
    ...
    def test_face_match_creation_with_all_fields(self):
    ...
    def test_face_match_no_match(self):
...
class TestBaseFaceRecognitionProvider:
    ...
    def test_init_initializes_empty_reference_encodings(self):
    ...
    def test_find_matches_in_image_no_faces_detected(self):
    ...
    def test_find_matches_in_image_faces_detected_no_match(self):
        """Test find_matches_in_image when faces are detected but don't match."""
        provider = ConcreteProvider({})

        # Set up mock detected faces
        face1 = FaceEncoding(encoding=np.array([0.5, 0.6, 0.7]), source="test.jpg")
    ...
    def test_find_matches_in_image_faces_detected_with_match(self):
        """Test find_matches_in_image when faces are detected and match."""
        provider = ConcreteProvider({})

        # Set up mock detected faces
        face1 = FaceEncoding(encoding=np.array([0.1, 0.2, 0.3]), source="test.jpg")
        provider.set_mock_detected_faces([face1])
    ...
    def test_find_matches_in_image_multiple_faces_mixed_results(self):
        """Test find_matches_in_image with multiple faces, some matching."""
        provider = ConcreteProvider({})

        # Set up multiple detected faces
        face1 = FaceEncoding(encoding=np.array([0.1, 0.2, 0.3]), source="test.jpg")
        face2 = FaceEncoding(encoding=np.array([0.4, 0.5, 0.6]), source="test.jpg")
        face3 = FaceEncoding(encoding=np.array([0.7, 0.8, 0.9]), source="test.jpg")
        provider.set_mock_detected_faces([face1, face2, face3])

        # Create a provider that returns alternating match results
        call_count = [0]

        def alternating_match(face_encoding, tolerance=0.6):
            call_count[0] += 1
        ...
        provider.compare_faces = alternating_match
    ...
    def test_find_matches_in_image_uses_custom_tolerance(self):
        ...
        def capture_tolerance(face_encoding, tolerance=0.6):
            captured_tolerance.append(tolerance)
            return FaceMatch(is_match=False, confidence=0.0, distance=1.0)

        provider.compare_faces = capture_tolerance
    ...
    def test_find_matches_in_image_uses_default_source(self):
    ...
    def test_concrete_provider_get_provider_name(self):
        """Test get_provider_name returns correct name."""
        provider = ConcreteProvider({})

        assert provider.get_provider_name() == "concrete_test"

    def test_concrete_provider_validate_configuration_valid(self):
    ...
    def test_concrete_provider_validate_configuration_invalid(self):
